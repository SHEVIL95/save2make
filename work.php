...
function register($con,$mail){
	...
	if(isset($_POST['submit']))
	{
		...
		if(mysqli_query($con,$insertQuery)){
			...
			if($success) {
				$success= "<p style='color:purple;font-size:15px;font-family:Impact'>Your account has been created Successfully!!<br/>
...
function pass_checker($data){
	if(strlen($data)<6){
		...
		$data="password must contain at least one capital letter";
		return $data;
	}
	else{
		$data="success";
...
function register2($con){
	...
	{	
		...
		{
			$result=(mysqli_query($con,"SELECT Password FROM users WHERE email='$email' And active='0'"));
