


<!DOCTYPE html>
<html>
<head>
	<title>save2make Donations</title>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	
<meta charset="utf-8">

	<link rel ="stylesheet" href="../css/style.css">
<body style="background:#eeeeee; margin:0px;">

	<div style="width:100%; height:50px; background:#cc0000; margin-bottom: 0px;">
		<div style="width:400px; line-height:50px; text-indent:18px;float:left;color:white; font-size:24px;font-family:arial;">
			<a href="#" style="color:white; text-decoration: none;">save2make</a>
		</div>
		<div style=" line-height:50px; text-indent:18px;float:left;color:white; font-size:12px;font-family:arial;">
				<a class="hyper"  href="admin_home.php" >HOME</a>&nbsp;&nbsp;&nbsp;&nbsp;
				<a class="hyper"  href="../ngo.php">PARTNERS</a>&nbsp;&nbsp;&nbsp;&nbsp;
				<a class="hyper"  href="admin_dashboard.php">ADMINISTRATION</a>&nbsp;&nbsp;&nbsp;&nbsp;
				<a class="hyper"  href="../about.php" >ABOUT PROJECT</a>&nbsp;&nbsp;&nbsp;&nbsp;
			</div>

			<div style=" line-height:50px; text-indent:18px;float:right;color:white; font-size:16px;font-family:arial;">	

				Welcome &nbsp;&nbsp;<span style="color:#EEDF75; text-transform: uppercase;font-weight:bold;"></span>
			</div>
	</div>

 </div>
 
 